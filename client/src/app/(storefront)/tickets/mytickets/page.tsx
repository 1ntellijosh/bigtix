...
/**
 * Looks up orders reserved in local storage, removes them if they have expired, and returns them as a record to show on
 * the my tickets page
 *
 * @returns {OrdersReservedRecord} - orders reserved record
 */
function readAndUpdateOrdersReserved(): OrdersReservedRecord {
...
/**
 * Writes orders reserved data to local storage
 *
...
export default function MyTicketsPage() {
  ...
  useEffect(() => {
    ...
    setTicketsOnOrder(readAndUpdateOrdersReserved());
  }, []);
  ...
  const handleOrderCreated = (resp: OrderCreatedResponse) => {
    ...
    const ordersReserved = readAndUpdateOrdersReserved();
    const expiresAtIso =
      resp.order.expiresAt != null ? new Date(resp.order.expiresAt).toISOString() : '';
    resp.tickets.forEach((t) => {
      ordersReserved[t.id] = { orderId: resp.order.id, expiresAt: expiresAtIso };
    });
    writeOrdersReserved(ordersReserved);
    setTicketsOnOrder(ordersReserved);
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3, alignItems: 'flex-start' }}>
        ...
        >
          <Typography variant="subtitle1" sx={{ mb: 1 }}>
            Subtotal ({inCartCount} {inCartCount === 1 ? 'item' : 'items'}): ${(inCartSubtotal / 100).toFixed(2)}
          </Typography>
          <Button
            variant="contained"
            color="primary"
          ...
          {reservedTickets.length === 0 ? (
            ...
            reservedTickets.map((ticket) => (
              <CartTicket
                ...
                variant="reserved"
                ticket={ticket}
                orderId={ticketsOnOrder[ticket.id]?.orderId ?? ''}
                expiresAtDisplay={formatReadableTime(ticketsOnOrder[ticket.id]?.expiresAt) ?? ''}
              />
            ))
          )}
        </Box>
