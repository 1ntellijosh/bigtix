/**
 * Initial page for selling tickets. You search for an event that you want to sell tickets for. When you click on the
 * event, it will take you to the ticket creation page.
 *
 * @since create-tickets--JP
 */
'use client';

import Container from '@mui/material/Container';
import Box from '@mui/material/Box';
import Typography from '@mui/material/Typography';
import EventSearch from '../../../../components/EventSearch';
import { API } from '../../../../lib/api/dicts/API';
import { useState, useMemo } from 'react';
import type { SavedEventDoc } from '../../../../lib/Types';
import Button from '@mui/material/Button';
import { useRouter } from 'next/navigation';
import { getDateSegments } from '../../../../lib/DateMethods';
import EventItem from '../../../../components/EventItem';
import TicketForm, { type TicketFormData } from '../../../../components/TicketForm';
import ListingCard from '../../../../components/ListingCard';
import Dialog from '@mui/material/Dialog';
import DialogTitle from '@mui/material/DialogTitle';
import DialogContent from '@mui/material/DialogContent';

export default function SellPageContent() {
  const router = useRouter();
  const [selectedEvent, setSelectedEvent] = useState<SavedEventDoc | null>(null);
  const [successfulCreate, setSuccessfulCreate] = useState(false);
  const [eventDescription, setEventDescription] = useState<string>('');
  const [eventDateSegments, setEventDateSegments] = useState<{ month: string, day: number, weekday: string } | null>(null);

  const [editedTicketIndex, setEditedTicketIndex] = useState<number | null>(null);
  const [newTicketData, setNewTicketData] = useState<TicketFormData | null>(null);
  const [newTickets, setNewTickets] = useState<TicketFormData[]>([]);

  const addTicket = (title: string) => {
    setNewTicketData({ title: title, priceCents: 0, description: '', serialNumber: '' });
  }

  const onEventSelected = async (event: any) => {
    const dateSegments = getDateSegments(new Date(event.date));
    setEventDateSegments(dateSegments);
    setEventDescription(`${event.location}`);

    const createdEvent = await API.tick!.createEvent!({ tmEventId: event.id }) as unknown as SavedEventDoc;

    setSelectedEvent(createdEvent);
    setNewTickets([]);
    addTicket(createdEvent.title);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const submitTickets = async () => {
    const tickets = [];
    for (const ticket of newTickets) {
      tickets.push({
        title: ticket.title,
        price: ticket.priceCents,
        description: ticket.description,
        serialNumber: ticket.serialNumber,
        eventId: selectedEvent?.id,
      });
    }

    await API.tick!.createTickets!({tickets: tickets});

    onSuccessfulCreate();
  }

  const onSuccessfulCreate = () => {
    setSuccessfulCreate(true);
  }

  const onSaveTicket = (ticket: TicketFormData) => {
    const tickets = [...newTickets];

    if (editedTicketIndex == null) tickets.push(ticket);
    else {
      tickets[editedTicketIndex] = ticket;
      setEditedTicketIndex(null);
    }

    setNewTickets(tickets);
    setNewTicketData(null);
  }

  const onCancelTicket = () => {
    if (newTickets.length === 0) {
      setNewTicketData(null);
      setNewTickets([]);
      setSelectedEvent(null);

      return;
    }
    
    setNewTicketData(null);
  }

  const onEditTicket = (index: number) => {
    const ticket = newTickets[index];
    setNewTicketData(ticket ?? null);
    setEditedTicketIndex(index);
  }

  const onDeleteTicket = (index: number) => {
    if (newTickets.length === 1) {
      setNewTicketData(null);
      setNewTickets([]);
      setSelectedEvent(null);

      return;
    }

    const tickets = [...newTickets];
    tickets.splice(index, 1);
    setNewTickets(tickets);
  }

  const allTicketsDone = useMemo(() => {
    return newTicketData == null && newTickets.length > 0;
  }, [newTicketData, newTickets]);

  return (
    <Container maxWidth="lg" disableGutters>
      <Box
        sx={{
          mt: 10,
          display: 'flex',
          flexDirection: 'column',
          justifyContent: 'center',
          alignItems: 'center',
        }}
      >
        {!selectedEvent ? (
          <Box sx={{ textAlign: 'center' }}>
            <Typography component="h1" sx={{ margin: 'auto', fontWeight: 400, fontFamily: 'oswald', fontSize: '40px' }}>
              Sell Your Tickets
            </Typography>
            <Typography component="h5" sx={{ margin: 'auto', mb: 2, fontWeight: 400, fontSize: '18px' }}>
              BigTix is the one-stop-shop for ticket buyers and sellers.
            </Typography>
          </Box>
        ) : (
          <Box sx={{
            width: '100%',
            maxWidth: '900px',
            display: 'flex',
            flexDirection: {
              xs: 'column',
              sm: 'column',
              md: 'row',
              lg: 'row',
              xl: 'row',
            },
            justifyContent: {
              xs: 'center',
              sm: 'center',
              md: successfulCreate ? 'center' : 'flex-start',
              lg: successfulCreate ? 'center' : 'flex-start',
              xl: successfulCreate ? 'center' : 'flex-start',
            },
            alignItems: {
              xs: 'center',
              sm: 'center',
              md: 'flex-start',
              lg: 'flex-start',
              xl: 'flex-start',
            },
          }}>
            <Box
              sx={{
                // marginTop: 'auto',
              }}>
              {!successfulCreate ? (
              <Typography component="h1" sx={{  fontWeight: 400, fontFamily: 'oswald', fontSize: '40px' }}>
                  Selling Tickets for:
                </Typography>
              ) : (
                <Typography component="h1" sx={{  fontWeight: 400, fontFamily: 'oswald', fontSize: '40px' }}>
                  All Done!
                </Typography>
              )}
            </Box>
            {selectedEvent && !successfulCreate ? (
              <Box sx={{
                maxWidth: '400px',
                marginLeft: {
                  xs: 0,
                  sm: 0,
                  md: 3,
                  lg: 3,
                  xl: 3,
                },
                display: 'flex',
                marginTop: {
                  xs: '10px',
                  sm: '10px',
                  md: 0,
                  lg: 0,
                  xl: 0,
                },
                justifyContent: {
                  xs: 'center',
                  sm: 'center',
                  md: 'flex-end',
                  lg: 'flex-end',
                  xl: 'flex-end',
                },
              }}>
                <EventItem size="small" name={selectedEvent.title} dateSegments={eventDateSegments!} description={eventDescription} />
              </Box>
            ) : null}
          </Box>
        )}

        <Box sx={{
          my: 2,
          width: '100%',
          // Set a max-width that increases at specific breakpoints
          maxWidth: {
            xs: '375px', // max-width on extra-small screens
            sm: '500px', // max-width on small screens
            md: '900px', // max-width on medium screens
            lg: '1200px', // max-width on large screens
          },
        }}>
          <Box
            sx={{
              visibility: selectedEvent ? 'hidden' : 'visible',
              height: selectedEvent ? '0px' : 'auto',
              maxWidth: '95%',
              margin: '0 auto'
            }}
          >
            <EventSearch
              onSelect={onEventSelected}
              options={{
                eventItemBtnLabel: 'Sell Tickets',
                searchPlaceholder: 'Search your event and start selling...'
              }}
            />
          </Box>

          {selectedEvent && !successfulCreate ? (
            <Box>
              <Dialog open={newTicketData != null} onClose={() => setNewTicketData(null)} maxWidth="sm" fullWidth>
                <DialogTitle>{selectedEvent.title}</DialogTitle>
                <DialogContent sx={{ pt: 0 }}>
                  <TicketForm
                    isCreatingNew={true}
                    initialTitle={newTicketData?.title ?? ''}
                    initialPriceCents={newTicketData?.priceCents ?? 0}
                    initialDescription={newTicketData?.description ?? ''}
                    initialSerialNumber={newTicketData?.serialNumber ?? ''}
                    onSave={async (ticket: TicketFormData) => await onSaveTicket(ticket)}
                    onCancel={() => onCancelTicket()}
                  />
                </DialogContent>
              </Dialog>
              {newTickets.map((ticket, index) => (
                <Box key={index} sx={{ mb: 2 }}>
                  <ListingCard
                    key={index}
                    listing={{ id: index.toString(), title: ticket.title, price: ticket.priceCents, description: ticket.description, order: null }}
                    showForSaleActions
                    onEditClick={() => onEditTicket(index)}
                    children={<Button sx={{ ml: 1 }} variant="outlined" color="secondary" size="small" onClick={() => onDeleteTicket(index)}>Delete</Button>}
                  />
                </Box>
              ))}

              <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', mt: 2 }}>
                <Button sx={{ mr: 1 }} variant="contained" color="primary" onClick={() => addTicket(selectedEvent?.title ?? '')}>+ Add Ticket</Button>

                <Button variant="contained" color="primary" onClick={() => submitTickets()} disabled={!allTicketsDone}>Done!</Button>
              </Box>
            </Box>
          ) : null}

          {successfulCreate ? (
            <Box sx={{ margin: 'auto', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
              <Typography component="h5" sx={{ textAlign: 'center', mb: 2, fontWeight: 400, fontSize: '20px' }}>
                Your tickets are entered for sale! We will email you a link to your tickets shortly. Click the button below to return to the home page.
              </Typography>
              <Box sx={{ mt: 2}}>
                <Button variant="contained" color="primary" onClick={() => router.push('/')}>
                  Return to Home Page
                </Button>
              </Box>
            </Box>
          ) : (
            null
          )}
        </Box>
      </Box>
    </Container>
  );
}
