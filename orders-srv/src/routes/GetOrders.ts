...
/**
 * Retrieves a single order's public safe details by id
 *
 * @param {string} id  The id of the order to retrieve
 *
 * @throws {BadRequestError}  If order is not valid
 * @throws {NotFoundError}  If order is not found
 */
router.get('/get-order-safe-details/:id', [ 
  param('id').trim().notEmpty().isMongoId().withMessage('ID is required'),
],
api.validateRequest,
api.getCurrentUser,
...
api.callAsync(async (req: Request, res: Response) => {
  ...
  res.status(STATUS_CODES.SUCCESS).send({ id: order.id, status: order.status, expiresAt: order.expiresAt });
})
);
