...
describe('Get tickets routes tests', () => {
  ...
  it('returns a 400 if the ticket id is not a valid MongoDB ID', async () => {
    const response = await request(tickApp).get('/api/tickets/get-ticket/not-a-valid-id');
  ...
  it('returns a 404 if the ticket is not found by id', async () => {
    ...
    const response = await request(tickApp).get(`/api/tickets/get-ticket/${invalidId}`);

    expect(response.status).toBe(404);
  });

  it('returns the ticket if it is found by id', async () => {
    const response = await request(tickApp).get(`/api/tickets/get-ticket/${savedTickets[0].id}`);
    ...
    expect(response.body.event).toBe(savedTickets[0].event!.toString());
  });
  ...
  it('returns the ticket if it is found by event id', async () => {
    const response = await request(tickApp).get(`/api/tickets/for-event/${validEventId}`);
    ...
    expect(response.body[0].event).toBe(savedTickets[0].event!.toString());
  });
